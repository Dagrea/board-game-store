...
class Registration extends React.Component {
	constructor(props) {
      super(props);
      this.state = {login: '', password: '',fullname: '', email: '',phone: '', city: '',address: ''};
    ...
    onSubmit(event){
      ...
      if (this.state.login === '' || this.state.password === '' || this.state.fullname === '' || this.state.email === ''
        || this.state.phone === '' || this.state.city === '' || this.state.address === '') {
        alert("Введите все данные");
        event.preventDefault();
        return ;
      }
      axios.post('http://localhost:3000/registration', {...state}).then(({data}) => {
    ...
    render() {
      return (
        ...
        <Form.Group widths='equal'>
          ...
          <Form.Input fluid label='Адресс' placeholder='Введите предположительное место доставки' name="address" 
